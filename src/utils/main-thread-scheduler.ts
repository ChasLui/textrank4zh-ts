...
import { TextRankResult, ErrorType } from '../types';
import { safeSync, safeAsync, ok, errOf } from './result-helpers';
...
export class MainThreadScheduler {
   ...
   * 调度执行任务（智能选择调度方式）
   */
  async scheduleTask<T>(taskFn: () => T | Promise<T>, options: SchedulerOptions = {}): Promise<TextRankResult<T>> {
    ...
    }, ErrorType.COMPUTATION_ERROR, { taskId, schedulingMethod });
    ...
    if (!executeResult.ok) {
      ...
      if (schedulingMethod === 'background-task') {
        const promiseResult = await safeAsync(() => this.executeWithPromise(task, options), ErrorType.COMPUTATION_ERROR, { taskId, fallback: 'promise' });
        if (!promiseResult.ok) {
          return await safeAsync(() => this.executeSync(task), ErrorType.COMPUTATION_ERROR, { taskId, fallback: 'sync' });
        }
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const executeWithIdleCallback = () => {
        ...
        if (this.capabilities.requestIdleCallback) {
          ...
          window.requestIdleCallback(
            (deadline) => {
              const executeResult = safeSync(() => {
                ...
                executeInIdleTime();
              }, ErrorType.COMPUTATION_ERROR, { taskId: task.id, method: 'requestIdleCallback' });
          ...
          }, ErrorType.COMPUTATION_ERROR, { taskId: task.id, method: 'postTask' });
  ...
  private async executeWithPromise<T>(
    ...
    const { timeSlice = 5, yieldInterval = 1000 } = options;
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const executeWithYielding = async () => {
        const executeResult = await safeAsync(async () => {
          ...
          return result;
        }, ErrorType.COMPUTATION_ERROR, { taskId: task.id, method: 'promise' });
  ...
  private async executeSync<T>(task: ScheduledTask<T>): Promise<T> {
    const executeResult = await safeAsync(async () => {
      ...
      return result;
    }, ErrorType.COMPUTATION_ERROR, { taskId: task.id, method: 'sync' });
  ...
  async scheduleBatch<T>(
    ...
    }, ErrorType.COMPUTATION_ERROR, { batchSize: tasks.length, priority });
  }
   ...
   * 检测当前主线程繁忙程度
   */
  async measureMainThreadBusyness(): Promise<TextRankResult<{
