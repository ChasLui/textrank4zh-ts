import type { DataTransferUtils, TransferableTextData, TextRankResult } from '../types';
import { WorkerType, ErrorType } from '../types';
import { safeSync, ok } from './result-helpers';
 ...
 */
function isArrayBufferRecord(value: unknown): value is Record<string, ArrayBuffer> {
...
export class WorkerDataTransfer implements DataTransferUtils {
  ...
  private detectTransferableSupport(): boolean {
    const result = safeSync(
      () => {
        ...
        // 尝试创建一个 ArrayBuffer，构造失败会被 safeSync 捕获
        new ArrayBuffer(1);
  ...
  private detectWorkerSupport(): boolean {
    const result = safeSync(() => typeof Worker !== 'undefined', ErrorType.UNSUPPORTED_ERROR, {
      feature: 'worker',
    });
    return result.isOk() ? result.value : false;
  }
  ...
  private manualTextToArrayBuffer(text: string): TextRankResult<ArrayBuffer> {
    return safeSync(
      () => {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          if (charcode < 0x80) utf8.push(charcode);
            ...
            // 孤立代理项（未配对的 0xD800-0xDFFF）不是合法 Unicode 标量值。
            // 原先它会落到下面的 3 字节分支，产出 WTF-8 而非合法 UTF-8，
            // 使得 TextEncoder 路径与手工路径对同一输入结果不同；
            // 现与 TextEncoder 一致替换为 U+FFFD（EF BF BD）
      ...
      ErrorType.SERIALIZATION_ERROR,
      { text: text.substring(0, 100), method: 'manual' }
    );
  }
  ...
  private manualArrayBufferToText(buffer: ArrayBuffer): TextRankResult<string> {
    return safeSync(
      () => {
        ...
        // 读取指定位置的字节；越界意味着多字节序列被截断，显式报错而不是产生 NaN
        const byteAt = (index: number): number => {
          const byte = bytes[index];
          if (byte === undefined) {
            throw new Error(`UTF-8 解码失败：字节序列在位置 ${index} 被截断`);
          }
          return byte;
        };
        ...
        while (i < length) {
          ...
          if (byte1 < 0x80) {
            ...
            // 非法前导字节（孤立续字节 0x80-0xBF，或 0xF8 及以上）。
            // 原先没有这个分支，字节被静默丢弃，导致输出比输入少字符且无任何提示；
            // 现与 TextDecoder 一致产出替换字符，保证两条解码路径结果相同
            result += '�';
          }
        }
        return result;
      ...
      ErrorType.SERIALIZATION_ERROR,
      { bufferSize: buffer.byteLength, method: 'manual' }
    );
  }
  ...
  deserializeFromArrayBuffer<T>(buffer: ArrayBuffer): TextRankResult<T> {
    ...
    return safeSync(() => JSON.parse(textResult.value) as T, ErrorType.SERIALIZATION_ERROR, {
      bufferSize: buffer.byteLength,
    });
  }
   ...
   * 批量反序列化数据
   */
  batchDeserialize<T extends Record<string, unknown>>(
    serializedData: Record<string, ArrayBuffer>
  ): TextRankResult<T> {
    return safeSync(
      () => {
   ...
   */
  shouldUseTransferable(data: unknown, threshold: number = 1024): TextRankResult<boolean> {
    // 首先检查环境支持
  ...
  prepareDataForTransfer(data: unknown): {
    ...
    // 非对象数据无法按键序列化，直接走传统传输
    if (useTransferable && isRecord(data)) {
      const serializeResult = this.batchSerialize(data);
      if (serializeResult.ok === false) {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            'TextRank4ZH-TS: Transferable 准备失败，降级到传统传输:',
  ...
  processReceivedData(data: unknown): unknown {
    if (isRecord(data) && data['__transferable'] === true) {
      ...
      // 载荷不是 ArrayBuffer 映射时无法反序列化，与失败分支一样返回原始数据
      if (!isArrayBufferRecord(serializedData)) {
   ...
   * 安全的批量反序列化（带错误处理）
   */
  safeBatchDeserialize<T extends Record<string, unknown>>(
    serializedData: Record<string, ArrayBuffer>
  ): {
    data: T | null;
    success: boolean;
