...
export class WordSegmentation {
  ...
  constructor(config: SegmentationConfig = {}) {
    const { 
      stopWords, 
      allowSpeechTags = DEFAULT_CONFIG.ALLOW_SPEECH_TAGS 
    } = config;
  ...
  private initJieba(): TextRankResult<void> {
    const result = safeSync(() => {
      ...
      this.jieba = jieba;
      debug('使用内置轻量级分词器');
    }, ErrorType.INITIALIZATION_ERROR, { component: 'jieba' });
  ...
  segment(
    ...
    if (useSpeechTagsFilter) {
      // 使用词性标注分词
      result = this.jieba.tag(text).map((item: any) => ({
        word: item.word,
