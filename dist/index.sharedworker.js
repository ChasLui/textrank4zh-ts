...
//#endregion
//#region src/worker/textrank.sharedworker.ts
var SharedWorkerManager = class {
	constructor() {
	...
	*/
	async handleMessage(connectionId, event) {
		...
		const handleResult = await safeAsync(async () => {
			const processedPayload = dataTransfer.processReceivedData(message.payload);
			if (message.type === "analyze_keywords" || message.type === "analyze_sentences") {
				...
				if (transferables && transferables.length > 0) connection.port.postMessage(response, transferables);
				else connection.port.postMessage(response);
