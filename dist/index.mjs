...
async function withTimeout(promise, timeoutMs, context) {
	...
	try {
		...
		if (error instanceof Error && error.message.includes("操作超时")) return Result.error(error);
		return Result.error(createError(ErrorType.COMPUTATION_ERROR, error instanceof Error ? error.message : String(error), error instanceof Error ? error : void 0, context));
	}
}
//#endregion
