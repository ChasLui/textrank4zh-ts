...
describe('pageRank', () => {
  ...
  test('不同alpha值应该产生不同结果', () => {
    ...
    const hasSignificantDifference = result1.scores.some((score, i) => 
      Math.abs(score - result2.scores[i]) > 0.001
    );
...
describe('buildWordGraph', () => {
  test('应该构建词图', () => {
    const vertexWords = [['苹果', '好吃'], ['香蕉', '甜']];
    const edgeWords = [['苹果', '好吃'], ['香蕉', '甜']];
...
describe('sortWords', () => {
  test('应该返回排序的关键词', () => {
    const vertexWords = [['北京', '首都'], ['上海', '城市']];
    const edgeWords = [['北京', '首都'], ['上海', '城市']];
...
describe('sortSentences', () => {
  test('应该返回排序的句子', () => {
    ...
    const words = [['北京', '首都'], ['上海', '大城市'], ['深圳', '发展', '很快']];
